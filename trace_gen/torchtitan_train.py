...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def train(self):
        ...
        def trace_handler(prof):
            logger.info(f"Inside kineto handler at step {self.step}")
            # prof.export_chrome_trace("kineto_trace.json")
            rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
            path = os.path.join(self.job_config.job.dump_folder, f"kineto_trace_rank{rank}.json")
            prof.export_chrome_trace(path)
            logger.info(f"Kineto trace exported to {path}")

        rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
        et_path = os.path.join(job_config.job.dump_folder, f"torch_et_rank{rank}.json")
        et = ExecutionTraceObserver()
        ...
        with (
            ...
            # ) as torch_profiler,
            torch_profiler_ctx as torch_profiler,
            maybe_enable_memory_snapshot(
                job_config.profiling,
                global_step=self.step,
                base_folder=job_config.job.dump_folder,
            ...
            while self.should_continue_training():
                ...
                if self.step == 7:
                    et.stop()
                    logger.info(f"Execution Trace stopped at step {self.step}")
...
def main(trainer_class: type[Trainer]) -> None:
    ...
    """
    init_logger()
    logger.info("On local version of TorchTitan train.py")

    import torchtitan

    if not hasattr(torchtitan, "__version__"):
        torchtitan.__version__ = "0.2.0"
    logger.info(
