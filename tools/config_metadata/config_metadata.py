...
def metric_cal(directory: str, profile_mode: str = "auto", workload_card_path: str | None = None) -> dict[str, Any]:
    ...
    if workload_card:
        ...
        if result["parallelism"].get("tp", 1) > 1 or result["parallelism"].get("pp", 1) > 1:
            print(f"  Config metadata: TP={result['parallelism'].get('tp')}, PP={result['parallelism'].get('pp')}, CP={result['parallelism'].get('cp')}", file=sys.stderr)
    ...
    if not workload_card:
        print(f"  Warning: No workload card (YAML or TOML) found in {trace_dir} or parent directories", file=sys.stderr)
        ...
        print(f"  Warning: Workload card found but world_size is 0. Parallelism config may be missing.", file=sys.stderr)
...
def _load_workload_card_from_path(card_path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        elif card_path.suffix.lower() == '.toml' and HAS_TOML:
...
def _load_workload_card(trace_dir: Path) -> dict[str, Any] | None:
    ...
    toml_names = [
        "*.toml",  # Any TOML file in config directory
    ]
