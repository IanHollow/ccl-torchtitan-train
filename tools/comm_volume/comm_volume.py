...
def metric_cal(directory: str, profile_mode: str = "auto") -> dict[str, Any]:
    ...
    Returns:
        Dictionary with communication volume metrics:
            ...
            - bytes_per_step: Average bytes per traixning step
    ...
    print(f"  Total: {total_bytes / (1024**3):.2f} GB ({gb_per_step:.3f} GB/step)", file=sys.stderr)
    print(f"  TP: {tp_bytes / (1024**3):.2f} GB ({fractions['tp'] * 100:.1f}%)", file=sys.stderr)
    print(f"  DP: {dp_bytes / (1024**3):.2f} GB ({fractions['dp'] * 100:.1f}%)", file=sys.stderr)
    print(f"  PP: {pp_bytes / (1024**3):.2f} GB ({fractions['pp'] * 100:.1f}%)", file=sys.stderr)
    print(f"  EP: {ep_bytes / (1024**3):.2f} GB ({fractions['ep'] * 100:.1f}%)", file=sys.stderr)
