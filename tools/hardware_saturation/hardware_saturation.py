...
try:
    ...
    try:
        ...
        HAS_TOML = False
        print("Warning: tomli not available. TOML config files cannot be read. Install with: pip install tomli", file=sys.stderr)


def metric_cal(directory: str, profile_mode: str = "auto", workload_card_path: str | None = None) -> dict[str, Any]:
    ...
    # Load workload card for model info - use explicit path if provided, otherwise search
    ...
    if not memory_data.get("memory_by_rank") and workload_card:
        ...
        if estimated_memory > 0:
            ...
            print(f"  Note: Using estimated memory from model: {estimated_memory:.1f} GB (memory profiling not enabled in traces)", file=sys.stderr)
    ...
    if workload_card:
        ...
        if not mfu_data or mfu_data.get("mfu_percent", 0) == 0:
            print(f"  Warning: MFU calculation returned 0 or failed. Check workload card and iteration time.", file=sys.stderr)
    else:
        print(f"  Warning: No workload card found. MFU/TFLOPs cannot be calculated.", file=sys.stderr)
        mfu_data = {
            "mfu_percent": 0.0,
            "tflops_per_gpu": 0.0,
    ...
    if result.get("mfu_percent", 0) > 0:
        print(f"  Hardware Saturation: MFU={result['mfu_percent']:.1f}%, TFLOPs={result.get('tflops_per_gpu', 0):.1f}", file=sys.stderr)
    if result.get("memory_by_rank"):
        print(f"  Memory: {len(result['memory_by_rank'])} ranks detected", file=sys.stderr)
    else:
        print(f"  Warning: No memory data found in traces", file=sys.stderr)
    ...
    try:
        if card_path.suffix.lower() in ['.yaml', '.yml']:
            ...
            if HAS_TOML:
                ...
                print(f"Warning: tomli not available. Cannot read TOML file: {card_path}", file=sys.stderr)
                print(f"         Install with: pip install tomli", file=sys.stderr)
...
def _convert_toml_to_workload_card(toml_data: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if parallel:
        result["parallelism"] = {
            ...
            "dp": parallel.get("data_parallel_shard_degree", 1) * parallel.get("data_parallel_replicate_degree", 1),
    ...
    if model:
        ...
        if "8b" in full_name.lower() or "8B" in full_name.lower():
            result["workload"]["model"]["num_params"] = 8e9
        elif "70b" in full_name.lower() or "70B" in full_name.lower():
            result["workload"]["model"]["num_params"] = 70e9
        elif "13b" in full_name.lower() or "13B" in full_name.lower():
            result["workload"]["model"]["num_params"] = 13e9
    
    # Extract hardware info (if available in TOML)
    # TOML configs typically don't have hardware section, but check anyway
...
def _extract_memory_usage(trace_dir: Path) -> dict[str, Any]:
    ...
    for trace_path in trace_files:
        ...
        try:
            ...
            # PyTorch profiler records memory in various event types
            # Common patterns:
            # - Events with cat="memory" 
            # - Events with name containing "Memory", "alloc", "reserve"
            # - Memory snapshot events (ph='i' for instant events)
            # - Events in "user_annotation" category with memory-related names
            
            for event in events:
                cat = event.get("cat", "")
                name = event.get("name", "")
                ph = event.get("ph", "")  # Phase: 'B' (begin), 'E' (end), 'X' (complete), 'i' (instant)
                args = event.get("args", {})
                
                # Look for memory events - check multiple patterns
                is_memory_event = (
                    cat == "memory" or 
                    cat == "user_annotation" and ("memory" in name.lower() or "Memory" in name) or
                    "memory" in name.lower() or
                    "Memory" in name or
                    "alloc" in name.lower() or
                    "reserve" in name.lower() or
                    name == "Memory Snapshot" or
                    name == "GPU Memory" or
                    "snapshot" in name.lower() and "memory" in name.lower()
                )
                ...
                if is_memory_event:
                    ...
                    for key in ["size", "bytes", "allocated_bytes", "reserved_bytes", 
                               "Allocated Bytes", "Reserved Bytes", "current_allocated", 
                               "current_reserved", "Size", "Bytes", "allocated", "reserved",
                               "Device Total Allocated", "Device Total Reserved",
                    ...
                    if size_bytes is None:
                        # Check for nested dicts like args["snapshot"] or args["memory"]
                ...
                # Also look for memory info in any event args (some profilers embed it everywhere)
                if not is_memory_event:
                    # Check for memory-related keys in any event's args
                    for key in ["Device Total Allocated", "Device Total Reserved", 
                               "total_allocated_bytes", "total_reserved_bytes"]:
                        if key in args:
                            val = args[key]
                            if isinstance(val, (int, float)) and val > 0:
    ...
    if not peak_memory_gb:
        print(f"  Warning: No memory data found in traces. Searched {len(trace_files)} trace files.", file=sys.stderr)
        if trace_files:
            # Try to inspect one trace file to see what events are available
            try:
                with trace_files[0].open() as f:
                    sample_data = json.load(f)
                    sample_events = sample_data.get("traceEvents", [])
                    # Count event categories
                    categories = {}
                    for evt in sample_events[:100]:  # Sample first 100 events
                        cat = evt.get("cat", "unknown")
                        categories[cat] = categories.get(cat, 0) + 1
                    print(f"    Sample trace file has {len(sample_events)} events with categories: {list(categories.keys())[:10]}", file=sys.stderr)
            except Exception:
                pass
...
def _extract_memory_from_snapshots(trace_files: list[Path]) -> tuple[dict[int, float], dict[int, float]]:
    ...
    for trace_path in trace_files:
        ...
        try:
            ...
            # Look for memory snapshot events - check multiple patterns
            for event in events:
                cat = event.get("cat", "")
                name = event.get("name", "")
                ph = event.get("ph", "")
                ...
                # Memory snapshots can be:
                # - Instant events (ph='i') with memory info
                # - Events with "Memory" or "Snapshot" in name
                # - Events in "memory" category
                is_snapshot = (
                    (ph == "i" and ("memory" in name.lower() or "Memory" in name)) or
                    "memory" in cat.lower() or 
                    "memory" in name.lower() or
                    "Memory" in name or
                    "snapshot" in name.lower() and ("memory" in name.lower() or "Memory" in name)
                )
                
                if is_snapshot:
                    # Try to get current memory usage from various field names
                    for key in ["Allocated Bytes", "Reserved Bytes", "allocated_bytes", 
                               "reserved_bytes", "current_allocated", "current_reserved",
                               "Device Total Allocated", "Device Total Reserved",
                               "total_allocated", "total_reserved", "size", "bytes"]:
                        if key in args:
                            size_bytes = args[key]
                    ...
                    if rank not in memory_samples or not memory_samples[rank]:
                        for nested_key in ["snapshot", "memory", "Memory", "allocated", "reserved"]:
                            if nested_key in args and isinstance(args[nested_key], dict):
                                nested = args[nested_key]
                                for key in ["Allocated Bytes", "Reserved Bytes", "allocated_bytes", 
                                          "reserved_bytes", "size", "bytes"]:
                                    if key in nested:
                                        size_bytes = nested[key]
                                        if isinstance(size_bytes, (int, float)) and size_bytes > 0:
                                            memory_gb = size_bytes / (1024 ** 3)
                                            if rank not in memory_samples:
                                                memory_samples[rank] = []
                                            memory_samples[rank].append(memory_gb)
                                            break
                                if rank in memory_samples and memory_samples[rank]:
                                    break
            ...
            if "memory" in metadata:
                ...
                if isinstance(mem_info, dict):
                    for key in ["allocated_bytes", "reserved_bytes", "total_allocated", "total_reserved"]:
                        if key in mem_info:
...
def _extract_rank_from_filename(path: Path) -> int:
    ...
    # Try patterns like "rank0", "rank_0", "kineto_trace_rank0", etc.
    import re
...
def _calculate_mfu(trace_dir: Path, workload_card: dict[str, Any]) -> dict[str, Any]:
    ...
    # This is a simplified calculation - real MFU needs exact FLOP counts
    ...
    if seq_len == 0 or batch_size == 0:
        print(f"  Warning: seq_len={seq_len}, batch_size={batch_size} from workload card", file=sys.stderr)
        return {
            "mfu_percent": 0.0,
            "tflops_per_gpu": 0.0,
    ...
    if iter_time_ms <= 0:
        print(f"  Warning: Could not determine iteration time from traces", file=sys.stderr)
        return {
            "mfu_percent": 0.0,
            "tflops_per_gpu": 0.0,
    ...
    print(f"  MFU calculation: seq_len={seq_len}, batch_size={batch_size}, iter_time={iter_time_ms:.1f}ms", file=sys.stderr)
    
    # Estimate model FLOPs per token
    # This is a rough estimate - real implementation would need exact model architecture
    # For transformer: ~2 * num_params * seq_len FLOPs per forward+backward
    ...
    if estimated_flops_per_token <= 0:
        print(f"  Warning: Could not estimate FLOPs per token (model_name={model_name})", file=sys.stderr)
        return {
            "mfu_percent": 0.0,
            "tflops_per_gpu": 0.0,
    ...
    # Note: DP (Data Parallel) doesn't reduce per-GPU FLOPs, it replicates computation
    ...
    # For transformer training: forward + backward = ~6 * num_params FLOPs per token
    ...
    if peak_tflops > 0:
        ...
        # Cap MFU at 100% (shouldn't exceed peak, but allow slight overage for measurement error)
        if mfu > 100:
            print(f"  Warning: MFU calculated as {mfu:.1f}%, capping at 100% (likely due to parallelism calculation or measurement error)", file=sys.stderr)
            mfu = min(mfu, 100.0)
    else:
        mfu = 0.0
...
def _get_avg_iter_time(trace_dir: Path) -> float:
    ...
    # This is a simplified version - in practice would use iter_time metric
...
def _estimate_flops_per_token(model_name: str, model_config: dict[str, Any]) -> float:
    ...
    # This is a very rough approximation - actual depends on architecture
...
def _get_num_gpus(workload_card: dict[str, Any]) -> int:
    """Get number of GPUs from workload card."""
    hardware = workload_card.get("hardware", {})
    num_gpus = hardware.get("num_gpus", 0)
    if num_gpus > 0:
        return num_gpus
    
    # Try parallelism config
    parallel = workload_card.get("parallelism", {})
